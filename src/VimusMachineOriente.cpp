...
void VimusMachineOriente::update()
{
    ...
    if(estado==ESTADO_APARECENDO){
        ...
        if(tempoPassadoMSegs>25) {
            ...
            if(tempoPassadoMSegs>INTERVALO_GIRANDO){
                opacidade=1-(tempoPassadoMSegs-INTERVALO_GIRANDO)/INTERVALO_DESAPARECENDO;
        ...
        } else if (audioSampler->getSecondOffset(1) > 195443) { //verificar se passou da parte legal do final
            audioSampler->setPlaybackPos(1, 195443);
        }
    }
...
void VimusMachineOriente::iniciaTeia() {
  ...
  for(int i=0; i<NUM_PONTOS_BASE_CENTRO; i++) {
    float teta = -1*TWO_PI*i/NUM_PONTOS_BASE_CENTRO;
