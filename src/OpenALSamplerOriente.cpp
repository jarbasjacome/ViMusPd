...
OpenALSamplerOriente::OpenALSamplerOriente()
{
	...
	if (!this->loadWaveBuffer("/dados/data/oriente/violao1234.wav", buffers[0]))
		alDeleteBuffers(NUM_BUFFERS, buffers);
	if (!this->loadWaveBuffer("/dados/data/oriente/violao_assovio.wav", buffers[1]))
		alDeleteBuffers(NUM_BUFFERS, buffers);
...
void OpenALSamplerOriente::displayOpenALError(char* _string, ALenum error)
{
	printf("\n");
	switch (error)
	{
...
int	OpenALSamplerOriente::loadWaveBuffer (char* _fileName, ALuint _buffer)
{
	int			error;
	ALenum		format;
	ALsizei		size;
...
void OpenALSamplerOriente::playSample (int sNumber)
{
	alSourcePlay(source[sNumber]);
}

void OpenALSamplerOriente::stopSample (int sNumber)
{
	alSourceStop(source[sNumber]);
}

void OpenALSamplerOriente::setSamplePitch (int sNumber, float pitch)
{
	alSourcef(source[sNumber],AL_PITCH,pitch);
}

void OpenALSamplerOriente::setPlaybackPos (int sNumber, float sec)
{
    alSourcef(source[sNumber],AL_SEC_OFFSET,sec);
}

void OpenALSamplerOriente::setGain(int sNumber, float gain)
{
    alSourcef(source[sNumber],AL_GAIN,gain);
}
ALfloat OpenALSamplerOriente::getSecondOffset(int sNumber)
{
    ALfloat pos = 0;
